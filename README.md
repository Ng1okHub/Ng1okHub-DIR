<p align="center">
  <img src="images/ng1ok_hub_blank.webp" alt="Ng1okHub_blank" />
</p>

---

「Ng1okHub」是我基於 Notion 搭建的付費知識平台，裡面主要會分享一些Android逆向相關的文章 / 工具 (偏遊戲方向)  
當前倉庫是「Ng1okHub」的目錄，簡單介紹了現有的文章和工具，有興趣可以聯繫我tg / vx: ngiokweng  
(注: 有效期是永久, 因此價格可能會隨Ng1okHub內容的價值而提升, 目前仍是原價)

## 📚 文章

- [Appdome 分析與繞過(完整)](articles/01_Appdome分析與繞過(完整)/readme.md)
- [U3D 手遊 Dump SDK 常規思路](articles/02_U3D手遊Dump%20SDK常規思路/readme.md)
- [如何快速繞過 Appdome 的注入檢測](articles/03_如何快速繞過Appdome的注入檢測/readme.md)
- [Frida 魔改集成 wxshadow 記錄](articles/04_Frida魔改集成wxshadow記錄/readme.md)
- [gcash有趣的注入檢測分析](articles/05_gcash有趣的注入檢測分析/readme.md)

## 🔧 工具

注: 工具只提供用法和二進制文件

- [nggtrace - 基於 QBDI 的調用流 trace 工具](tools/nggtrace%20-%20基於QBDI的調用流trace工具/readme.md)
- [內核模塊 - 隱藏 (s)maps 中的特徵](tools/內核模塊%20-%20隱藏(s)maps中的特徵/readme.md)
- [魔改 Frida - 集成 wxshadow 無痕 Hook](tools/魔改Frida%20-%20集成wxshadow無痕Hook/readme.md)
